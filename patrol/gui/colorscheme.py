...
def _colorscheme_name_to_class(signal):  # pylint: disable=too-many-branches
    ...
    if usecustom:
        if os.path.exists(signalcliconfpath('colorschemes')):
            initpy = signalcliconfpath('colorschemes', '__init__.py')
    ...
    if usecustom and \
            exists(signalcliconfpath('colorschemes', scheme_name)):
        scheme_supermodule = 'colorschemes'
    elif exists(signalclirelpath('colorschemes', scheme_name)):
...
def get_all_colorschemes(cli):
    ...
    confpath = cli.confpath('colorschemes')
