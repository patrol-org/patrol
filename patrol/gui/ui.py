...
class UI(  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def __init__(self, env=None, cli=None):  # pylint: disable=super-init-not-called
    ...
    def suspend(self):
        ...
        if self.cli.image_displayer:
            self.cli.image_displayer.quit()
    ...
    def destroy(self):
        ...
        if 'vcsthread' in self.__dict__:
            if not self.vcsthread.stop():
                self.cli.notify('Failed to stop `UI.vcsthread`', bad=True)
        ...
        if key == 27 or (128 <= key < 256):
            ...
            if key >= 0:
                ...
                if key == curses.KEY_MOUSE:
                    ...
                    if not self.cli.input_is_blocked():
                        self.handle_key(key)
    ...
    def draw(self):
        ...
        if self._draw_title and self.settings.update_title:
            cwd = self.cli.thisdir.path
            if self.settings.tilde_in_titlebar \
               and (cwd == self.cli.home_path
                    or cwd.startswith(self.cli.home_path + "/")):
                cwd = '~' + cwd[len(self.cli.home_path):]
            ...
            try:
                if _in_tmux():
                    ...
                    if tmux_allow_rename == 'off':
                        self.cli.notify('Warning: allow-rename not set in Tmux!',
                                       bad=True)
                    else:
                ...
                self.cli.notify("Couldn't access previous multiplexer window"
    ...
    def restore_multiplexer_name(self):
        if self._multiplexer_title:
            try:
                ...
                self.cli.notify("Could not restore multiplexer window name!",
                               bad=True)
