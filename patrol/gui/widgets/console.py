...
class Console(Widget):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def __init__(self, win):
        ...
        if not ranger.args.clean:
            self.historypath = self.cli.datapath('history')
    ...
    def press(self, key):
        self.cli.ui.keymaps.use_keymap('console')
        if not self.cli.ui.press(key):
            self.type_key(key)
    ...
    def transpose_subr(self, line, x, y):
        ...
        if not 0 <= x[0] < x[1] <= y[0] < y[1] <= len(line):
            self.cli.notify("Tried to transpose invalid regions.", bad=True)
            return line
    ...
    def execute(self, cmd=None):
        ...
        if cmd:
            ...
            self.cli.execute_console(self.line)
    ...
    def _get_cmd(self, quiet=False):
        try:
            ...
            if not quiet:
                self.cli.notify("Command not found: `%s'" % self.line.split()[0], bad=True)
            return None
        return command_class(self.line)

    def get_cmd_class(self):
        return self.cli.commands.get_command(self.line.split()[0], abbrev=True)
    ...
    def _get_tab(self, tabnum):
        ...
        return self.cli.commands.command_generator(self.line)
