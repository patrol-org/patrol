...
class Pager(Widget):  # pylint: disable=too-many-instance-attributes
    ...
    def clear_image(self, force=False):
        if (force or self.need_clear_image) and self.image_drawn:
            self.cli.image_displayer.clear(self.x, self.y, self.wid, self.hei)
    ...
    def finalize(self):
        self.cli.ui.win.move(self.y, self.x)
    ...
    def draw_image(self):
        if self.image and self.need_redraw_image:
            ...
            try:
                self.cli.image_displayer.draw(self.image, self.x, self.y,
                                             self.wid, self.hei)
            except ImgDisplayUnsupportedException as ex:
                self.cli.settings.preview_images = False
                self.cli.notify(ex, bad=True)
    ...
    def press(self, key):
        self.cli.ui.keymaps.use_keymap('pager')
        self.cli.ui.press(key)
        ...
        while True:
            try:
                ...
                for part in ((0,) if not
                             self.cli.settings.wrap_plaintext_previews else
