...
class TaskView(Widget, Accumulator):
    ...
    def finalize(self):
        ...
        self.cli.ui.win.move(y, self.x)

    def task_remove(self, i=None):
        if i is None:
            i = self.pointer

        if self.cli.loader.queue:
            self.cli.loader.remove(index=i)

    def task_move(self, to, i=None):  # pylint: disable=invalid-name
        if i is None:
            i = self.pointer

        self.cli.loader.move(pos_src=i, pos_dest=to)

    def press(self, key):
        self.cli.ui.keymaps.use_keymap('taskview')
        self.cli.ui.press(key)

    def get_list(self):
        return self.cli.loader.queue
