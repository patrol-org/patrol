...
class StatusBar(Widget):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, win, column=None):
        ...
        self.cli.signal_bind('tab.layoutchange', self.request_redraw, weak=True)
        self.cli.signal_bind('setop.viewmode', self.request_redraw, weak=True)
    ...
    def draw(self):  # pylint: disable=too-many-branches
        ...
        if self.column != self.cli.ui.browser.main_column:
            self.column = self.cli.ui.browser.main_column
            self.need_redraw = True
        ...
        if self.cli.thisfile:
            self.cli.thisfile.load_if_outdated()
            try:
                ctime = self.cli.thisfile.stat.st_ctime
        ...
        if self.old_du and not self.cli.thisdir.disk_usage:
            self.old_du = self.cli.thisdir.disk_usage
            self.need_redraw = True

        if self.old_thisfile != self.cli.thisfile:
    ...
    def _get_left_part(self, bar):  # pylint: disable=too-many-branches,too-many-statements
        ...
        if self.column is not None and self.column.target is not None\
            ...
            directory = self.cli.thistab.at_level(0)
            if directory:
        ...
        if self.cli.mode != 'normal':
            perms = '--%s--' % self.cli.mode.upper()
        else:
        ...
        directory = target if target.is_directory else \
            targetcliget_directory(os.path.dirname(target.path))
    ...
    def _get_right_part(self, bar):  # pylint: disable=too-many-branches,too-many-statements
        ...
        if self.cli.thisdir.flat:
            right.add("flat=", base, 'flat')
            right.add(str(self.cli.thisdir.flat), base, 'flat')
            right.add(", ", "space")

        if self.cli.thisdir.narrow_filter:
            right.add("narrowed")
            right.add(", ", "space")

        if self.cli.thisdir.filter:
            right.add("f=`", base, 'filter')
            right.add(self.cli.thisdir.filter.pattern, base, 'filter')
            right.add("', ", "space")
