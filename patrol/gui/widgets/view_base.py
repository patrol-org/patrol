...
class ViewBase(Widget, DisplayableContainer):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, win):  # pylint: disable=super-init-not-called
        ...
        self.cli.signal_bind('move', self.request_clear)
    ...
    def draw(self):
        ...
        for tab in self.cli.tabs.values():
    ...
    def finalize(self):
        if self.pager is not None and self.pager.visible:
            try:
                self.cli.ui.win.move(self.main_column.y, self.main_column.x)
            except curses.error:
                pass
        else:
            ...
            try:
                self.cli.ui.win.move(col_y, col_x)
            except curses.error:
                pass

    def _draw_bookmarks(self):
        self.columns[-1].clear_image(force=True)
        self.cli.bookmarks.update_if_outdated()
        self.color_reset()
        self.need_clear = True

        sorted_bookmarks = sorted(
            (
                item for item in self.cli.bookmarks
                if self.cli.settings.show_hidden_bookmarks
    ...
    def _draw_hints(self):
        ...
        populate_hints(self.cli.ui.keybuffer.pointer)
        ...
        def sort_hints(hints):
            ...
            if len(hints) > self.cli.settings.hint_collapse_threshold:
