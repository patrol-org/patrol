...
class BrowserColumn(Pager):  # pylint: disable=too-many-instance-attributes
    ...
    def click(self, event):     # pylint: disable=too-many-branches
        ...
        if self.target is None:
            ...
            if self.target.accessible and self.target.content_loaded:
                ...
                if direction:
                    if self.level == -1:
                        self.cli.move_parent(direction)
                    else:
                        return False
                elif event.pressed(1):
                    if not self.main_column:
                        self.cli.enter_dir(self.target.path)

                    if index < len(self.target):
                        self.cli.move(to=index)
                    ...
                    try:
                        ...
                        if clicked_file.is_directory:
                            self.cli.enter_dir(clicked_file.path, remember=True)
                        elif self.level == 0:
                            self.cli.thisdir.move_to_obj(clicked_file)
                            self.cli.execute_file(clicked_file)
        elif self.target.is_file:
            if event.pressed(3):
                self.cli.execute_file(self.target)
            else:
                self.scrollbit(direction)
        else:
            if self.level > 0 and not direction:
                self.cli.move(right=0)
    ...
    def _draw_directory(  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
        ...
        if self.cli.ui.viewmode == 'multipane' and self.tab is not None:
            active_pane = self.tab == self.cli.thistab
            if active_pane:
        ...
        copied = [f.path for f in self.cli.copy_buffer]
        ...
        for line in range(self.hei):
            ...
            tagged = self.cli.tags and drawn.realpath in self.cli.tags
            if tagged:
                tagged_marker = self.cli.tags.marker(drawn.realpath)
            else:
                tagged_marker = " "
            ...
            if current_linemode.uses_metadata:
                metadata = self.cli.metadata.get_metadata(drawn.path)
            ...
            key = (self.wid, selected_i == i, drawn.marked, self.main_column,
                   ...
                   self.cli.do_cut, current_linemode.name, metakey, active_pane,
    ...
    def _get_index_of_selected_file(self):
        if self.cli.ui.viewmode == 'multipane' and self.tab != self.cli.thistab:
    ...
    def _draw_directory_color(self, i, drawn, copied):
        ...
        if self.cli.tags and drawn.realpath in self.cli.tags:
        ...
        if drawn.path in copied:
            this_color.append('cut' if self.cli.do_cut else 'copied')
