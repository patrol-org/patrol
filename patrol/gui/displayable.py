...
class Displayable(  # pylint: disable=too-many-instance-attributes
    ...
    Read-Only: (i.e. recommended not to change manually)
        ...
        settings, cli -- inherited shared variables
    """

    def __init__(self, win,  # pylint: disable=super-init-not-called
                 env=None, cli=None, settings=None):
    ...
    def resize(self, y, x, hei=None, wid=None):
        ...
        do_move = True
        try:
            maxy, maxx = self.cli.ui.termsize
        except TypeError:
            pass
        else:
            if hei is None:
            ...
            if x < 0 or y < 0:
                self.cli.notify("Warning: Subwindow origin below zero for <%s> "
...
class DisplayableContainer(Displayable):
    ...
    def __init__(self, win, env=None, cli=None, settings=None):
        if env is not None:
            self.env = env
        if cli is not None:
            self.cli = cli
