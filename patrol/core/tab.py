...
class Tab(CLIAware, SettingsAware):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, path):
        ...
        self.cli.signal_bind('move', self._set_thisfile_from_signal,
                            priority=settings.SIGNAL_PRIORITY_AFTER_SYNC,
                            weak=(PY3))
        self.cli.signal_bind('tab.change', self._on_tab_change,
                            weak=(PY3))

    def _set_thisfile_from_signal(self, signal):
        if self == signal.tab:
            self._thisfile = signal.new
            if self == self.cli.thistab:
    ...
    def _set_thisfile(self, value):
        if value is not self._thisfile:
            ...
            self.cli.signal_emit('move', previous=previous, new=value, tab=self)
    ...
    def enter_dir(self, path, history=True):
        ...
        if self.cli.settings.clear_filters_on_dir_change and self.thisdir:
        ...
        if not isdir(path):
            selectfile = path
            path = dirname(path)
        new_thisdir = self.cli.get_directory(path)
        ...
        if path == '/':
            self.pathway = (self.cli.get_directory('/'), )
        else:
            pathway = []
            currentpath = '/'
            for comp in path.split('/'):
                currentpath = join(currentpath, comp)
                pathway.append(self.cli.get_directory(currentpath))
            self.pathway = tuple(pathway)

        self.assign_cursor_positions_for_subdirs()

        # set the current file.
        self.thisdir.sort_directories_first = self.cli.settings.sort_directories_first
        self.thisdir.sort_reverse = self.cli.settings.sort_reverse
        ...
        self.cli.signal_emit('cd', previous=previous, new=self.thisdir)
