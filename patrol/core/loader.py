...
from ranger.core.shared import CLIAware
...
class CopyLoader(Loadable, CLIAware):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, copy_buffer, do_cut=False, overwrite=False, dest=None,
        ...
        self.original_path = dest if dest is not None else self.cli.thistab.path
    ...
    def generate(self):
        ...
        if self.do_cut:
            ...
            for fobj in self.copy_buffer:
                for path in self.cli.tags.tags:
                    if path == fobj.path or str(path).startswith(fobj.path):
                        tag = self.cli.tags.tags[path]
                        self.cli.tags.remove(path)
                        new_path = path.replace(
                            fobj.path,
                            os.path.join(self.original_path, fobj.basename))
                        self.cli.tags.tags[new_path] = tag
                        self.cli.tags.dump()
                n = 0
        ...
        cwd = self.cli.get_directory(self.original_path)
        cwd.load_content()


class CommandLoader(  # pylint: disable=too-many-instance-attributes
        Loadable, SignalDispatcher, CLIAware):
...
class Loader(CLIAware):
    """
    The Manager of 'Loadable' objects, referenced as cli.loader
    """
    ...
    def add(self, obj, append=False):
        ...
        self.cli.signal_emit("loader.before", loadable=obj, cli=self.cli)
        if self.paused:
            obj.pause()
        else:
            obj.unpause()
    ...
    def remove(self, item=None, index=None):
        ...
        if index is not None:
            ...
            if hasattr(item, 'unload'):
                item.unload()
            self.cli.signal_emit("loader.destroy", loadable=item, cli=self.cli)
            item.destroy()
            del self.queue[index]
            if item.progressbar_supported:
                self.cli.ui.status.request_redraw()
    ...
    def _remove_current_process(self, item):
        ...
        self.cli.signal_emit("loader.after", loadable=item, cli=self.cli)
        if item.progressbar_supported:
            self.cli.ui.status.request_redraw()
