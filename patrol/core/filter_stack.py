...
@stack_filter("mime")
class MimeFilter(BaseFilter, CLIAware):
    def __init__(self, pattern):
        self.regex = re.compile(pattern)

    def __call__(self, fobj):
        mimetype, _ = self.cli.mimetypes.guess_type(fobj.relative_path)
        if mimetype is None:
            return False
...
@stack_filter("hash")
class HashFilter(BaseFilter, CLIAware):
    def __init__(self, filepath=None):
        if filepath is None:
            self.filepath = self.cli.thisfile.path
        else:
            self.filepath = filepath
        if self.filepath is None:
            self.cli.notify("Error: No file selected for hashing!", bad=True)
...
class DuplicateFilter(BaseFilter, CLIAware):
    def __init__(self, _):
    ...
    def get_duplicates(self):
        duplicates = set()
        for dups in group_by_hash(self.cli.thisdir.files_all):
            if len(dups) >= 2:
...
@stack_filter("unique")
class UniqueFilter(BaseFilter, CLIAware):
    def __init__(self, _):
