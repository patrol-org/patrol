...
"""The Cli, putting the pieces together"""
...
class CLI(Actions,  # pylint: disable=too-many-instance-attributes
         SignalDispatcher):
    input_blocked = False
    ...
    def initialize(self):  # pylint: disable=too-many-statements
        ...
        self.settings.signal_bind(
            ...
            lambda signal: signal.cli.previews.clear(),
        )
        ...
        self.run = Runner(ui=self.ui, logfunc=mylogfunc, cli=self)

        self.settings.signal_bind(
            'setopt.metadata_deep_search',
            lambda signal: setattr(signal.cli.metadata, 'deep_search', signal.value)
        )
        self.settings.signal_bind(
            'setopt.save_backtick_bookmark',
            lambda signal: signal.cli.bookmarks.enable_saving_backtick_bookmark(signal.value)
        )
    ...
    def get_filesystem_objects(self, paths):
        ...
        for dirname, basenames in groups.items():
            directory = self.cli.get_directory(dirname)
            ...
            if basenames != set():
                ...
                self.cli.notify('Error: No such file or directory: {0}'.format(
                    names), bad=True)
                return None
        return result
