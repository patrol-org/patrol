...
"""The main function responsible to initialize the CLI object and stuff."""
...
def main(
    ...
    """initialize objects and run the cli"""
    import ranger.api
    from ranger.container.settings import Settings
    from ranger.core.shared import CLIAware, SettingsAware
    from ranger.core.cli import CLI
    ...
    if args.copy_config is not None:
        cli = CLI()
        cli.copy_config_files(args.copy_config)
        return 0
    if args.list_tagged_files:
        ...
        try:
            with open(
                cli.datapath('tagged'), 'r', encoding="utf-8", errors='replace'
            ) as fobj:
    ...
    try:  # pylint: disable=too-many-nested-blocks
        ...
        if cli.username == 'root':
            cli.settings.preview_files = False
            cli.settings.use_preview_script = False
        ...
        if not args.clean:
            ...
            # Restore saved tabs
            tabs_datapath = cli.datapath('tabs')
            if cli.settings.save_tabs_on_exit and os.path.exists(tabs_datapath) and not args.paths:
                try:
                    with open(tabs_datapath, 'r', encoding="utf-8") as fobj:
                        tabs_saved = fobj.read().partition('\0\0')
                        startup_path = cli.start_paths.pop(0)
                        cli.start_paths += tabs_saved[0].split('\0')
                        # Remove dead entries if this behavior is defined in settings
                        if cli.settings.filter_dead_tabs_on_startup:
                            cli.start_paths = list(filter(os.path.isdir, cli.start_paths))
                        try:
                            startup_path_tab_index = cli.start_paths.index(startup_path)
                        except ValueError:
                            cli.start_paths.insert(0, startup_path)
                    if tabs_saved[-1]:
        ...
        # Run the cli
        cli.initialize()
        cli.tab_move(startup_path_tab_index)
        ranger.api.hook_init(cli)
        cli.ui.initialize()

        if args.selectfile:
            cli.select_file(args.selectfile)

        if args.cmd:
            cli.enter_dir(cli.thistab.path)
            for command in args.cmd:
                cli.execute_console(command)

        if int(os.environ[level]) > 1:
            warning = 'Warning:'
            nested_warning = "You're in a nested ranger instance!"
            warn_when_nested = cli.settings.nested_ranger_warning.lower()
            if warn_when_nested == 'true':
                cli.notify(' '.join((warning, nested_warning)), bad=False)
            elif warn_when_nested == 'error':
                cli.notify(' '.join((warning.upper(), nested_warning + '!!')),
                          bad=True)

        if ranger.args.profile:
            import cProfile
            import pstats
            ranger.__cli = cli  # pylint: disable=protected-access
            profile_file = tempfile.gettempdir() + '/ranger_profile'
            cProfile.run('ranger.__cli.loop()', profile_file)
        ...
        exit_msg += '\n'.join(VERSION_MSG) + '\n'
        try:
            exit_msg += "Current file: {0}\n".format(repr(cli.thisfile.path))
        except Exception:  # pylint: disable=broad-except
            pass
        exit_msg += '''
{0}
ranger crashed. Please report this traceback at:
https://github.com/patrol-org/patrol/issues
'''.format(ex_traceback)
...
def load_settings(  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
    ...
    cli.commands = ranger.api.commands.CommandContainer()
    include = [name for name in dir(Actions) if name not in COMMANDS_EXCLUDE]
    cli.commands.load_commands_from_object(cli, include)
    cli.commands.load_commands_from_module(commands_default)
    ...
    if not clean:
                ...
                if os.path.exists(custom_comm_path):
                    try:
                        commands_custom = import_file('commands',
                                                      custom_comm_path)
                        cli.commands.load_commands_from_module(commands_custom)
        ...
        try:
            ...
            if not os.path.exists(cli.confpath('plugins', '__init__.py')):
                LOG.debug("Creating missing '__init__.py' file in plugin folder")
                with open(
                    cli.confpath('plugins', '__init__.py'), 'w', encoding="utf-8"
                ):
        ...
        if (os.environ.get('RANGER_LOAD_DEFAULT_RC', 'TRUE').upper() != 'FALSE'
            ...
            cli.source(default_conf)
        if system_conf_is_readable:
            cli.source(system_conf)
        if custom_conf_is_readable:
            cli.source(custom_conf)

    else:
        cli.source(cli.relpath('config', 'rc.conf'))
