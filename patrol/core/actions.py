...
class Actions(  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def reset(self):
        ...
        Reset the cli, clearing the directory buffer, reload rifle config
        """
    ...
    def change_mode(self, mode=None):
        ...
        """
        if mode is None:
            self.cli.notify('Syntax: change_mode <mode>', bad=True)
            return
    ...
    def _parse_option_value(  # pylint: disable=too-many-return-statements
            self, name, value):
        types = self.cli.settings.types_of(name)
        if bool in types:
    ...
    def execute_console(self, string='',  # pylint: disable=redefined-outer-name
        ...
        if cmd.resolve_macros and _MacroTemplate.delimiter in cmd.line:
            ...
            def anypath_macro(i, char):
                try:
                    val = self.cli.bookmarks[key_to_string(char)]
                except KeyError:
                    val = MACRO_FAIL
    ...
    def get_macros(self):
        ...
        if not ranger.args.clean:
            macros['confdir'] = self.cli.confpath()
            macros['datadir'] = self.cli.datapath()
        macros['space'] = ' '

        macros['f'] = lambda: self.cli.thisfile.relative_path

        macros['p'] = lambda: [os.path.join(self.cli.thisdir.path,
                                            fl.relative_path) for fl in
                               self.cli.thistab.get_selection()]
        macros['s'] = lambda: [fl.relative_path for fl in
                               self.cli.thistab.get_selection()]

        macros['c'] = lambda: [fl.path for fl in self.cli.copy_buffer]

        macros['t'] = lambda: [fl.relative_path for fl in
                               self.cli.thisdir.files if fl.realpath in
                               self.cli.tags]

        macros['d'] = macro_val(lambda: self.cli.thisdir.path, fallback='.')
        ...
        next_tab = None
        first_tab = None
        for tabname in self.cli.tabs:
            if not first_tab:
                first_tab = tabname
            if found_current_tab:
                next_tab = self.cli.tabs[tabname]
                break
            if self.cli.current_tab == tabname:
                found_current_tab = True
        if found_current_tab and next_tab is None:
            next_tab = self.cli.tabs[first_tab]
        try:
        ...
        if mode == 0 and 'label' not in kw:
            if ranger.args.choosefile:
                with open(
                    ...
                    fobj.write(self.cli.thisfile.path)

            if ranger.args.choosefiles:
                paths = []
                for hist in self.cli.thistab.history:
                    for fobj in hist.files:
                        if fobj.marked and fobj.path not in paths:
                            paths += [fobj.path]
                paths += [f.path for f in self.cli.thistab.get_selection() if f.path not in paths]
    ...
    def move(self,  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
        ...
        """
        cwd = self.thisdir
        kw.setdefault('cycle', self.cli.settings['wrap_scroll'])
        kw.setdefault('one_indexed', self.cli.settings['one_indexed'])
    ...
    def get_preview(self, fobj, width, height):
        ...
        try:
            ...
            self.cli.notify("Preview script `{0}` doesn't exist!".format(
                self.settings.preview_script), bad=True)
            return None

        if not stat_.st_mode & S_IEXEC:
            self.cli.notify("Preview script `{0}` is not executable!".format(
                self.settings.preview_script), bad=True)
            return None
    ...
    def tab_switch(self, path, create_directory=False):
        ...
        if not os.path.exists(path):
            ...
            if create_directory:
                ...
                if os.path.exists(potential_parent) and os.path.isdir(potential_parent):
                    ...
                    self.cli.notify("Unable to resolve given path.", bad=True)
                    return
        ...
        for name in self.cli.tabs:
            tab = self.cli.tabs[name]
            # Is a tab already open?
            if tab.path == target_directory:
                self.cli.tab_open(name=name)
                if file_selection:
                    self.cli.select_file(file_selection)
                return

        self.cli.tab_new(path=target_directory)
        if file_selection:
            self.cli.select_file(file_selection)
    ...
    def dump_keybindings(self, *contexts):
        ...
        for context in contexts:
            ...
            if context in self.cli.ui.keymaps:
                recurse([], self.cli.ui.keymaps[context])
            else:
                write("  None\n")
            write("\n")
    ...
    def paste_symlink(self, relative=False, make_safe_path=get_safe_path):
        ...
        for fobj in copied_files:
            ...
            try:
                if relative:
                    relative_symlink(fobj.path, join(self.cli.thisdir.path, new_name))
                else:
                    symlink(fobj.path, join(self.cli.thisdir.path, new_name))
    ...
    def paste_hardlink(self, make_safe_path=get_safe_path):
        for fobj in self.copy_buffer:
            new_name = make_safe_path(fobj.basename)
            try:
                link(fobj.path, join(self.cli.thisdir.path, new_name))
            except OSError as ex:
    ...
    def paste_hardlinked_subtree(self, make_safe_path=get_safe_path):
        for fobj in self.copy_buffer:
            try:
                target_path = join(self.cli.thisdir.path, fobj.basename)
    ...
    def delete(self, files=None):
        ...
        # COMPAT: old command.py use cli.delete() without arguments
        if files is None:
        ...
        for path in files:
            ...
            for tag in self.cli.tags.tags:
                if str(tag).startswith(path):
                    self.cli.tags.remove(tag)
