...
class CommandContainer(CLIAware):
    ...
    def alias(self, name, full_command):
        ...
        if cmd_cls is None:
            self.cli.notify('alias failed: No such command: {0}'.format(cmd_name), bad=True)
        else:
...
class Command(CLIAware):
    ...
    def _tab_directory_content(self):  # pylint: disable=too-many-locals
        ...
        try:
            directory = self.cli.get_directory(abs_dest)
            ...
            if rel_dest.endswith('/') or rel_dest == '':
                if directory.content_loaded:
                    ...
                    if self.cli.thisfile.basename in names:
                        i = names.index(self.cli.thisfile.basename)
        ...
        def execute(self):
            ...
            try:
                ...
                if ranger.args.debug:
                    raise
                self.cli.notify("Bad arguments for %s: %s, %s" % (func.__name__, args, kwargs),
                               bad=True)
