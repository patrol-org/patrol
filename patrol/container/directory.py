...
class Directory(  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def load_bit_by_bit(self):
        ...
        try:  # pylint: disable=too-many-nested-blocks
            if self.runnable:
                ...
                if self.cumulative_size_calculated:
                    ...
                    if self.content_loaded:
                        if self.cli.settings.autoupdate_cumulative_size:
                ...
                for name in filenames:
                    ...
                    if is_a_dir:
                        item = self.cli.get_directory(name, preload=stats, path_is_abs=True,
            ...
            self.loading = False
            self.cli.signal_emit("finished_loading_dir", directory=self)
            if self.vcs:
                self.cli.ui.vcsthread.process(self)
    ...
    def load_content(self, schedule=None):
        ...
        if not self.loading:
            ...
            if self.load_generator is None:
                ...
                if schedule and self.cli:
                    self.cli.loader.add(self)
                else:
                    for _ in self.load_generator:
                        pass
                    self.load_generator = None

            elif not schedule or not self.cli:
                for _ in self.load_generator:
                    pass
                self.load_generator = None
    ...
    def size(self):  # pylint: disable=method-hidden
        try:
            if self.cli.settings.automatically_count_files:
    ...
    def correct_pointer(self):
        ...
        if self == self.cli.thisdir:
            try:
                self.cli.thisfile = self.pointed_obj
            except AttributeError:
                pass
    ...
    def go(self, history=True):  # pylint: disable=invalid-name
        """enter the directory if the cli is running"""
        if self.cli:
            return self.cli.enter_dir(self.path, history=history)
        return False
