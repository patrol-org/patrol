...
class FileSystemObject(  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def linemode(self):  # pylint: disable=method-hidden
        ...
        for method, argument, linemode in self.cli.default_linemodes:
            if linemode in self.linemode_dict:
                if method == "always":
                    return linemode
                if method == "path" and argument.search(self.path):
                    return linemode
                if method == "tag" and self.realpath in self.cli.tags and \
                        self.cli.tags.marker(self.realpath) in argument:
                    return linemode
    ...
    def set_mimetype(self):
        ...
        self._mimetype = self.cli.mimetypes.guess_type(bname, False)[0]
    ...
    def mark(self, _):
        directory = self.cli.get_directory(self.dirname)
    ...
    def load(self):  # pylint: disable=too-many-statements
        ...
        self.display_data = {}
        self.cli.update_preview(self.path)
