...
class File(FileSystemObject):
    ...
    def has_preview(self):  # pylint: disable=too-many-return-statements
        if not self.cli.settings.preview_files:
            return False
        if self.is_socket or self.is_fifo or self.is_device:
            return False
        if not self.accessible:
            return False
        if self.cli.settings.preview_max_size and \
                self.size > self.cli.settings.preview_max_size:
            return False
        if self.cli.settings.preview_script and \
                self.cli.settings.use_preview_script:
            return True
        if self.container:
            return False
    ...
    def get_preview_source(self, width, height):
        return self.cli.get_preview(self, width, height)

    def is_image_preview(self):
        try:
            return self.cli.previews[self.realpath]['imagepreview']
        except KeyError:
            return False
