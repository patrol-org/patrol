...
SIGNAL_PRIORITY_BETWEEN = 0.6  # sanitized signal.value, old cli.settings.XYZ
SIGNAL_PRIORITY_SYNC = 0.2  # (Internal) updating cli.settings.XYZ
SIGNAL_PRIORITY_AFTER_SYNC = 0.1  # after cli.settings.XYZ was updated
...
class Settings(SignalDispatcher, CLIAware):
        ...
        if name == 'column_ratios':
            ...
            if isinstance(value, str):
                ...
                if os.path.exists(result):
                    ...
                    self.cli.notify("Preview script `{0}` doesn't exist!".format(result), bad=True)
                    signal.value = None

        elif name == 'use_preview_script':
            if self._settings.get('preview_script') is None and value and self.cli.ui.is_on:
                self.cli.notify("Preview script undefined or not found!",
                               bad=True)
    ...
    def _get_default(self, name):
        if name == 'preview_script':
            if ranger.args.clean:
                value = self.cli.relpath('data/scope.sh')
            else:
                value = self.cli.confpath('scope.sh')
    ...
    def get(self, name, path=None):
        ...
        if path:
            ...
            try:
                localpath = self.cli.thisdir.path
        ...
        if self._tagsettings and path:
            ...
            if realpath in self.cli.tags:
                tag = self.cli.tags.marker(realpath)
