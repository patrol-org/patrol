    ...
    def init_state(self, dirobj):
        ...
        if self.root:
            if self.is_root:
                ...
                self.rootvcs = dirobj.cli.get_directory(self.root).vcs
...
class VcsRoot(Vcs):  # pylint: disable=abstract-method
    ...
    def init_root(self):
        ...
        try:
            ...
            self.obj.cli.notify('VCS Exception: View log for more info', bad=True, exception=ex)
            return False
        self.rootinit = True
        return True
    ...
    def _update_walk(self, path, purge):  # pylint: disable=too-many-branches
        ...
        for wroot, wdirs, _ in os.walk(path):
            ...
            try:
                wrootobj = self.obj.cli.directories[wroot]
            except KeyError:
                wdirs[:] = []
                continue
            ...
            for wdir in list(wdirs):
                try:
                    wdirobj = self.obj.cli.directories[os.path.join(wroot, wdir)]
                except KeyError:
                    wdirs.remove(wdir)
                    continue
    ...
    def update_tree(self, purge=False):
        ...
        for path in list(self.links):
            ...
            try:
                dirobj = self.obj.cli.directories[path]
            except KeyError:
    ...
    def check_outdated(self):
        ...
        for wroot, wdirs, _ in os.walk(self.path):
            wrootobj = self.obj.cli.get_directory(wroot)
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                self._uiclinotify('VCS Exception: View log for more info', bad=True, exception=ex)
