...
class W3MImageDisplayer(ImageDisplayer, CLIAware):
    ...
    def draw(self, path, start_x, start_y, width, height):
        ...
        if self.cli.settings.w3m_delay > 0:
            from time import sleep
            sleep(self.cli.settings.w3m_delay)
    ...
    def _generate_w3m_input(self, path, start_x, start_y, max_width, max_height):
        ...
        start_x = int((start_x - 0.2) * fontw) + self.cli.settings.w3m_offset
        start_y = (start_y * fonth) + self.cli.settings.w3m_offset
...
# TODO: remove CLIAwareness, as stuff in ranger.ext should be
# ranger-independent libraries.


@register_image_displayer("iterm2")
class ITerm2ImageDisplayer(ImageDisplayer, CLIAware):
    ...
    def _fit_width(self, width, height, max_cols, max_rows):
        font_width = self.cli.settings.iterm2_font_width
        font_height = self.cli.settings.iterm2_font_height
...
class SixelImageDisplayer(ImageDisplayer, CLIAware):
    """Implementation of ImageDisplayer using SIXEL."""

    def __init__(self):
        self.win = None
        self.cache = {}
        self.cli.signal_bind('preview.cleared', lambda signal: self._clear_cache(signal.path))
    ...
    def _sixel_cache(self, path, width, height):
        ...
        if cacheable not in self.cache:
            ...
            sixel_dithering = self.cli.settings.sixel_dithering
    ...
    def draw(self, path, start_x, start_y, width, height):
        if self.win is None:
            self.win = self.cli.ui.win.subwin(height, width, start_y, start_x)
        else:
...
class TerminologyImageDisplayer(ImageDisplayer, CLIAware):
...
class URXVTImageDisplayer(ImageDisplayer, CLIAware):
    ...
    def _get_sizes(self):
        ...
        """
        if self.cli.ui.pager.visible:
            return self._get_max_sizes()

        total_columns_ratio = sum(self.cli.settings.column_ratios)
        preview_column_ratio = self.cli.settings.column_ratios[-1]
...
class KittyImageDisplayer(ImageDisplayer, CLIAware):
